...
public:
    ...
    pca9685_pub(nh.advertise<"msg/12servo.msg">("/servo_drive",10)),
    loop_rate(20)
    {
        initPCA9685Twist();
    }
...
// need this kind of topic for each servo
    void move_CB(const geometry_msgs::Twist &cmd_vel)
    {
        ...
        if(cmd_vel.angular.x == 0)
        {
            ...
            pca9685_msg.angular.z = 0;
        }
        else if(cmd_vel.angular.x == 1)
        {
            pca9685_msg.linear.x = cmd_vel.linear.x;
            pca9685_msg.angular.z = cmd_vel.angular.z;
        }
        else if(cmd_vel.angular.x == 2)
        {
            pca9685_msg.linear.x = cmd_vel.linear.x;
            pca9685_msg.angular.z = cmd_vel.angular.z;
        }
